...
#include "encoders_and_rfinder.h"
#include "flickers.h"
#include "net_USART.h"
#include "net_TWI.h"
#include "KHC.h"
...
ISR(TIMER0_OVF_vect)
{
	...
	cli();
	if (mainTimerCounter == 122) check_offline_state(FROM_TIMER); // раз в секунду проверяем оффлайн
...
static uint8_t devAddr, devType;
	...
	*/
	...
	switch (devType)
	{
		case 0x01:
		{
			...
			//таймер 2 (8 бит): режим normal, делитель 1024
			TCCR2 = 0x00;
